...
const animateFromEnd = () => {
  ...
  const interval = setInterval(() => {
    if (drawn[pointer].length < toDisplay[pointer].length) {
      ...
      process.stdout.write(ansiEscapes.cursorTo(currElement.x, pointer + 1));
      process.stdout.write(currElement.char);
    }
