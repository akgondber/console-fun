...
const drawTwo = () => {
  writeRel(2, 2, figureSet.lineUpDoubleRightDouble);
  // process.stdout.write(ansiEscapes.cursorMove(-1, -1));
...
const drawFive = () => {
  goToRel(4, 0);
  writeRel(-2, 0, figureSet.lineDownDoubleLeftDouble);
  times((i) => {
    // process.stdout.write(ansiEscapes.cursorMove(-1, 1));
    if (i !== 0) {
      writeRel(-2, 0, figureSet.lineDouble);
...
const traction = async (movingType = "chaotic-devouring") => {
  ...
  process.stdin.on("keypress", async (_chunk, key) => {
    if (true) {
      if (key.name === "n") {
        process.stdout.write(ansiEscapes.cursorBackward(2));
